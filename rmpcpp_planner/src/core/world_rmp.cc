...
rmpcpp::NVBloxWorldRMP<Space>::getPolicies() {
  ...
  /** Target Policy */
  policies.push_back(this->target_policy);

  /** 'World Policy' */
  switch (this->parameters.policy_type) {
    case SIMPLE_ESDF:
      world_policy = std::make_shared<rmpcpp::SimpleEsdfPolicy<Space>>(
          this->parameters.worldPolicyParameters, this->esdf_layer.get());
      break;
    case RAYCASTING_CUDA:
      world_policy = std::make_shared<rmpcpp::RaycastingCudaPolicy<Space>>(
          this->parameters.worldPolicyParameters, this->tsdf_layer.get(), this);
      break;
  }
  policies.push_back(world_policy);
  return policies;
}
...
void rmpcpp::NVBloxWorldRMP<Space>::setGoal(const Vector &new_goal) {
  ...
  this->goal = new_goal;
  this->target_policy = std::make_shared<rmpcpp::SimpleTargetPolicy<Space>>(
      new_goal, Eigen::Matrix<double, Space::dim, Space::dim>::Identity(),
      this->parameters.alpha_target, this->parameters.beta_target,
      this->parameters.c_softmax_target);
}
